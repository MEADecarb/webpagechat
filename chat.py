...
BASE_URL = st.secrets["https://energy.maryland.gov/Pages/default.aspx"]
...
for link in st.session_state.links:
    if st.button(link):
        page_content, page_links = scrape_website(link)
        st.success(f"Content loaded from {link}")
        st.session_state.content = page_content  # Update the content for the chatbot
        st.session_state.links = page_links  # Update the links
